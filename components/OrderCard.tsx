    ...
    <TouchableOpacity onPress={() => router.push({ pathname: '/(coins)/[id]', params: { id: order?.id } })} className={`w-full flex flex-row items-center justify-between gap-1 mb-4 relative  border-b border-b-bgDark py-3`} >
      <View className='flex flex-row gap-2'>
        ...
        <View className='flex flex-col gap-1'>
          <Text className='text-lg text-white font-JakartaBold '>{order?.name?.length > 8 ? `${order?.name?.slice(0, 8)}..` : order?.name}</Text>
      ...
      <View className='flex flex-col items-end justify-end'>
        <Text className='text-lg  font-bold text-white'>${order.current_price.toLocaleString()}</Text>
        <Text className={`text-xs  flex  font-bold ${order.price_change_percentage_24h >= 0 ? 'text-textGreen' : 'text-textRed'}`}>{order.price_change_percentage_24h.toFixed(2)}%</Text>
      </View>
    </TouchableOpacity>
  )
}
